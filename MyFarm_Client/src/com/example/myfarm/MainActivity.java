...
public class MainActivity extends Activity implements OnClickListener {
	...
	private String server = "192.168.0.29";
	...
	public static String sndMessage;
	...
	private Handler mmHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (soc_flag && sndMessage != null) {
				try {
					outs.write(sndMessage.getBytes("UTF-8"));
					outs.flush();
				} catch (IOException e) {
			...
			if (rcvthread.socRec_flag) {
				...
				temp_value = rcvthread.temperature;
				int temp = Integer.valueOf(temp_value);

				tv_temp.setText(String.format("%.1f", temp * 0.1));

				// 테스트
				// updateTemp(set_test_temp, set_low_temp, set_high_temp);
			...
			if (rcvthread.socRec_flag) {
				...
				tv_lux.setText("" + rcvthread.lux); // 조도값 출력
			...
			mmHandler.sendEmptyMessageDelayed(0, 100);
		}
	};
	...
	public void onClick(View v) {
		...
		case R.id.buttonConnect:
			...
			// editTextIPAddress.getWindowToken(), 0);
		...
		case R.id.buttonSend:
			...
			String sndOpkey = "CMD";
			sndOpkey = editTextIPAddress.getText().toString();
